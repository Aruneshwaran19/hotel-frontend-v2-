...
import { downloadBillingPdf } from "./whatsappBill";
...
export const generateInvoicePDF = async (props) => {
  ...
  if (billId) {
    try {
      if (action === "download") {
        await downloadBillingPdf(billId, selectedBill.customer_name);
        return;
      }

      const response = await auth.get(`/billings/${billId}/pdf`, {
        responseType: "blob",
      });
      ...
      console.error("Server invoice PDF failed, falling back to client render:", error);
    }
  }
