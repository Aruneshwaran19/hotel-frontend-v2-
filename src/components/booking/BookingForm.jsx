...
const dateOnlyFromDateTime = (dateTimeValue) =>
  dateTimeValue ? dateTimeValue.split("T")[0] : "";
...
const getDefaultDateTimes = (clickedDate) => {
  ...
  // Check-in at 1:00 PM; check-out at 11:00 AM on the next day
...
export default function BookingForm({
  ...
  const hasAutoAdvancedCheckIn = useRef(Boolean(initialData?.check_in));
  ...
  const handleCheckInChange = (e) => {
    ...
    const prevDate = dateOnlyFromDateTime(form.check_in);
    const newDate = dateOnlyFromDateTime(value);
    const dateChanged = Boolean(newDate && newDate !== prevDate);

    setForm((prev) => ({ ...prev, check_in: value }));

    if (!hasAutoAdvancedCheckIn.current && dateChanged) {
      hasAutoAdvancedCheckIn.current = true;
      e.target.blur();
      window.setTimeout(() => {
